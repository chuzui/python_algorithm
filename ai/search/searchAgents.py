...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        for corner in state.cornersVisted:
            if corner == False:
                return False
        return True
        util.raiseNotDefined()
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                newStata = copy.copy(state)
                newStata.position = nextx, nexty
...
class Graph:
    def __init__(self, V):
        self.V = V
        self.matrix = [[0 for x in range(V)] for y in range(V)]

    def addEdge(self, u, w, weight):
    ...
    def distance(self):
        ...
        while not s.isEmpty():
            ...
            for j in range(self.V):
                if self.matrix[i][j] != 0 and marked[j] == False:
                    if minLengh == 0:
                        minLengh = self.matrix[i][j]
                        minTar = j
                    elif self.matrix[i][j] < minLengh:
                        minLengh = self.matrix[i][j]
                        minTar = j
            length += minLengh
            if minLengh != 0:
...
def cornersHeuristic(state, problem):
    ...
    #     count = 1
    #     print walls
    #     print str(state.position[0]) + "        " + str(state.position[1])
    # print walls
    # print str(state.position[0]) + "        " + str(state.position[1])
    V = 5
    # for corner in state.cornersVisted:
