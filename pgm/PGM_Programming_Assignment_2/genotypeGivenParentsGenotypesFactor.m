...
len = length(genotypesToAlleles);
genotypeFactor.card = [len, len, len];
genotypeFactor.val = zeros(1, prod(genotypeFactor.card));

assignments = IndexToAssignment(1:length(genotypeFactor.val), genotypeFactor.card);
val = [];
child = genotypesToAlleles(assignments(:, 1), :);
father = genotypesToAlleles(assignments(:, 2), :);
mother = genotypesToAlleles(assignments(:, 3), :);
for i = 1:length(assignments)
    if child(i,1) == child(i, 2)
        fatherRate = length(find(father(i,:) == child(i,1))) / 2;
        motherRate = length(find(mother(i,:) == child(i,1))) / 2;
        val = [val, fatherRate * motherRate];
    else
        f1 = length(find(father(i,:) == child(i,1))) / 2;
        f2 = length(find(father(i,:) == child(i,2))) / 2;
        m1 = length(find(mother(i,:) == child(i,1))) / 2;
        m2 = length(find(mother(i,:) == child(i,2))) / 2;
        val = [val, f1 * m2 + f2 * m1];
    end
end

genotypeFactor = SetValueOfAssignment(genotypeFactor, assignments, val);
