...
vals = zeros(1,length(phenotypeFactor.val));
for i = 1:length(assignments)
    if assignments(i,3) > assignments(i,2)
        index = find(genotypesToAlleles(:,1) == assignments(i,2) & genotypesToAlleles(:,2) == assignments(i,3));
    else
        index = find(genotypesToAlleles(:,1) == assignments(i,3) & genotypesToAlleles(:,2) == assignments(i,2));
    end
    if assignments(i,1) == 1
        vals(i) = alphaList(index);
    else
        vals(i) = 1-alphaList(index);
    end
end
phenotypeFactor = SetValueOfAssignment(phenotypeFactor, assignments, vals);
