...
def add(u,v):
    ...
    assert u.D == v.D
    keys = set(u.f.keys()) | set(v.f.keys())
    return Vec(u.D, {key:getitem(u, key) + getitem(v, key) for key in keys})

def dot(u,v):
    "Returns the dot product of the two vectors"
    assert u.D == v.D
    keys = set(u.f.keys()) & set(v.f.keys())
    return sum([getitem(u, key) * getitem(v, key) for key in keys])

def scalar_mul(v, alpha):
    "Returns the scalar-vector product alpha times v"
    return Vec(v.D, {key:getitem(v, key) * alpha for key in v.f})

def neg(v):
    "Returns the negation of a vector"
    return Vec(v.D, {key:getitem(v, key) * -1 for key in v.f})
